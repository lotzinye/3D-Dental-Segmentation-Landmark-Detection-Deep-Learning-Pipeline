...
class InferencePipeLine:
    ...
    def __call__(self, stl_path):
        ...
        # Propagate per-vertex labels to sampled points via nearest-neighbour
        _tree = KDTree(org_feats[:, :3], leaf_size=2)
        _nn_idx = _tree.query(sampled_feats[:, :3], k=1, return_distance=False).reshape(-1)
        sampled_sem_labels = result_sem_labels.reshape(-1)[_nn_idx]
        sampled_ins_labels = result_ins_labels.reshape(-1)[_nn_idx]

        return {
            "sem":         result_sem_labels.reshape(-1),
            "ins":         result_ins_labels.reshape(-1),
            # Extra fields used by Stage 2 (landmark detection)
            "sampled_xyz":        sampled_feats[:, :3],   # (24000, 3) in TGNet norm space
            "sampled_feats":      sampled_feats,           # (24000, 6) [X,Y,Z,Nx,Ny,Nz]
            "sampled_sem_labels": sampled_sem_labels,      # (24000,) FDI labels for sampled pts
            "sampled_ins_labels": sampled_ins_labels,      # (24000,) instance IDs for sampled pts
        }
    ...
    def get_boundary_sampled_feats(self,point_labels, org_feats, sampled_feats, sample_output_features):
        ...
        non_bd_org_feat_cpu, non_bd_org_ps_label_cpu = gu.resample_pcd([non_bd_org_feat_cpu, non_bd_org_ps_label_cpu], 
                                                                       self.config["boundary_sampling_info"]["num_of_all_points"]-bd_org_feat_cpu.shape[0], "fps")
...
# ---------------------------------------------------------------------------
# Clean wrapper used by combined_pipeline.py
# ---------------------------------------------------------------------------

class InferencePipeline:
    """
    Thin wrapper around InferencePipeLine with a friendlier API.

    Args:
        fps_ckpt : path to the Stage-1 (FPS) TGNet checkpoint
        bdl_ckpt : path to the Stage-2 (BDL boundary) TGNet checkpoint
        device   : 'cuda' or 'cpu'
    """

    # Standard TGNet model hyper-parameters (must match the checkpoint)
    _FPS_MODEL_PARAMS = {
        "input_feat": 6,
        "stride":    [1, 4, 4, 4, 4],
        "nstride":   [2, 2, 2, 2],
    ...
    def __init__(self, fps_ckpt: str, bdl_ckpt: str, device: str = "cuda"):
        config = {
            "fps_model_info": {
                "model_parameter": self._FPS_MODEL_PARAMS,
                "load_ckpt_path": fps_ckpt,
            },
            "boundary_model_info": {
                "model_parameter": self._BDL_MODEL_PARAMS,
                "load_ckpt_path": bdl_ckpt,
            },
        ...
        self._pipeline = InferencePipeLine(config)
        self.device = device

    def run(self, obj_path: str) -> dict:
        """
        Run TGNet segmentation on a single jaw scan.

        Returns dict with keys:
            'jaw'          : 'upper' or 'lower' (detected from filename)
            'labels'       : (N,) int array of per-vertex FDI labels
            'instances'    : (N,) int array of instance IDs
            'sampled_xyz'  : (24000, 3) float32 in TGNet normalised space
            'sampled_feats': (24000, 6) float32 [X,Y,Z,Nx,Ny,Nz]
        """
        import os
        result = self._pipeline(obj_path)

        # Jaw detection from filename (same heuristic as predict_utils.py)
        basename = os.path.splitext(os.path.basename(obj_path))[0].lower()
        if "upper" in basename:
            jaw = "upper"
        elif "lower" in basename:
            jaw = "lower"
        else:
            jaw = "unknown"

        # Apply lower-jaw FDI offset (mirrors predict_utils.py:ScanSegmentation.predict)
        # InferencePipeLine.__call__ always outputs FDI 11-28; lower jaw needs 31-48.
        labels = result["sem"]
        sampled_sem = result["sampled_sem_labels"]
        if jaw == "lower":
            labels[labels > 0] += 20
            sampled_sem[sampled_sem > 0] += 20

        return {
            "jaw":                jaw,
            "labels":             labels,
            "instances":          result["ins"],
            "sampled_xyz":        result["sampled_xyz"],
            "sampled_feats":      result["sampled_feats"],
            "sampled_sem_labels": sampled_sem,
            "sampled_ins_labels": result["sampled_ins_labels"],
        }


# Alias for clarity
InferencePipeLineTGN = InferencePipeLine
